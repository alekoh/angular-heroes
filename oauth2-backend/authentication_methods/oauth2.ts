...
let GrantCode = require('../model/grant-code');
let AccessToken = require('../model/access-token');
    ...
    server.authorization(function(applicationID, redirectURI, done) {
        ...
        Application.findOne({ oauth_id: applicationID }, function(error, application) {
            ...
            if (application) {
                ...
                for (let i = 0; i < application.allowed_domains.length; i++) {
...
server.grant(oauth2orize.grant.code(function(application, redirectURI, user, ares, done) {
    let date = new Date();
    let time = date - (date.getTimezoneOffset() * 60000);
    let myTime = new Date(time);

    let grant = new GrantCode({
        user: user,
    ...
    grant.save(function (err) {
        if (err) { console.log("Error saving code!"); }

        else { done( null, grant.code ); }
    });
}));
